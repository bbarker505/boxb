...
library(leafsync) # Sync two maps (current year + last year)
...
# Current and previous year and date 
current_year <- 2021
...
#current_date <- as.Date(paste0("Jul-26-", current_year), format = "%b-%d-%Y") 
threeDays_date <- format(current_date + 3, format = "%b-%d-%Y")
fourDays_date <- format(current_date + 4, format = "%b-%d-%Y")
lastYr_date <- format(as.Date(gsub(current_year, last_year, current_date)), "%b-%d-%Y")

#current_date <- format(Sys.Date(), format = "%b-%d-%Y")
#lastYr_date <- gsub(current_year, last_year, current_date)
#threeDays_date <- format((Sys.Date() + 3), format = "%b-%d-%Y")
#fourDays_date <- format((Sys.Date() + 4), format = "%b-%d-%Y")

# Import model outputs ----

# File names
fls <- c("Cum_Inf_Risk_Total.tif", "Cum_Inf_Risk_3day.tif", "Cum_Inf_Risk_4day.tif")
outdir_current <- paste0("https://raw.githubusercontent.com/bbarker505/BOXB-webapp/main/rasters/", current_year)
outdir_lastYr <- paste0("https://raw.githubusercontent.com/bbarker505/BOXB-webapp/main/rasters/", last_year)
...
Rast_Import <- function(file_name) {
...
# Model outputs for current run (apply function)
rasts_current <- map(
  fls, function(file_name) {
    Rast_Import(paste0(outdir_current, "/", file_name))
  }
)
...
rasts_lastYr <- map(
  fls, function(file_name) {
    Rast_Import(paste0(outdir_lastYr, "/", file_name))
  }
)
...
#### * Pop-over messages ####
info_maptype <- gsub("\n", "", paste0("Two Day and Three Day risk maps depict the 
risk that has accumulated over the past two and three days, respectively. 
For example, today is ", current_date, ", so the three day risk map depicts risk 
that has accumulated since ", threeDays_date, ". The Cumulative (Total) risk map 
shows the total amount of risk that has accumulated since the start of 
the year (i.e., ", paste0("Jan-01-", current_year, ").  Hover the mouse over 
a location on the map and a risk value will appear in the upper left-hand covern
of the map.")))

info_address <- gsub("\n", "", "Marks, zooms to, and extracts risk information 
for a specified location. After checking the box, enter a city name, address, or 
place in the white box and press Submit.")

info_lastYr <- gsub("\n", "", "Produces a second map that shows risk for the same 
time last year. This may provide insight into how climate differences between years 
affects infection risk. The two risk maps are synced, so panning and zooming one 
map will do the same for the other.")
...
ui <- fluidPage(
  ...
  dashboardPage(
    dashboardHeader(
      title =  span("Boxwood Blight Risk Mapping Tool for Western OR and WA", 
                    style = "font-size: 21px;"),
      titleWidth = "700px"),
    dashboardSidebar(
      tags$style(HTML(".main-sidebar { color: black;font-size: 22px; }
                      .treeview-menu>li>a { color: black!important;font-size: 20px; }")),
      width = 200,
      sidebarMenu(
        menuItem(strong("Mapping App"), tabName = "risk_maps"),
        menuItem(strong("Station Apps"), tabName = "site_apps",
                 menuSubItem("Single Station", 
                             href = "https://uspest.org/risk/boxwood_app/"),
                 menuSubItem("Synoptic (Many Stations)",
                             href = "https://uspest.org/risk/boxwood_map/")))),
        ...
        box(title = tags$p(strong("Introduction", style = "font-size:22px")),
            ...
            fluidRow(
              ...
              column(width = 2, 
                     ...
                     img(src = "https://raw.githubusercontent.com/bbarker505/BOXB-webapp/main/images/boxb_infected_leaves.png", width = "100%")),
              column(width = 10, 
                     offset = 0, 
